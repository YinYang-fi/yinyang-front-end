...
const WalletButton: React.FC<WalletButtonProps> = (props) => {
  ...
  const address = account ? account?.substr(0, 6) + "..." + account?.substr(account.length - 4, 4) : "Not connected"
  ...
  useEffect(() => {
    ...
    if (!account && localAccount) {
      ...
      if (localAccount && walletProvider === "binancewallet") {
        handleConnectBinanceWallet();
      }
    }
  }, [account, handleConnectMetamask, handleConnectWalletConnect, handleConnectBinanceWallet]);
    ...
    <>
      <StyledWalletButton>
        {!userAccount ? (
          ...
          <Button onClick={handleWalletClick} size="sm" text={address} variant="tertiary" />
        )}
      </StyledWalletButton>
